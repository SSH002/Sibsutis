% ¬ариант 11.
% «адание 2.
% Ќайдите в файле все слова максимальной длины.
% —формируйте новый файл из найденных слов.

%ѕредикат преобразовани€ заданной строки в список слов
str_to_list_words(S, [Hw | Tn]):-
    front_token(S, W, S2), not(S2 = " "), !,
    Hw = W, str_to_list_words(S2, Tn).
    str_to_list_words(_, []).

%ѕредикат, который выдел€ет слова из строки0.
front_token(S, W, L1):-
